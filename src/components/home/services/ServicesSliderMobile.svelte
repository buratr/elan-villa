...
<div class="overflow-hidden">
  <div class="slider-container">
      ...
      {#each slides as slide, index (slide)}
        <div class={`slide ${getClass(index)}`}>
          <div class="flex flex-col items-center gap-4 text-center">
            ...
            >
              <LordIcon src={slide.src ? slide.src : undefined} name="{slide.icon}" size="100" class="text-white" />
            </div>
    ...
    <div class="flex justify-between px-8 z-20 absolute w-full bottom-8">
      <button on:click={() => handleArrowClick("prev")}>
        <IconArrowLeft
          width="32"
          class="stroke-purple-secondary !w-auto !h-auto stroke-9 cursor-pointer top-1/2 !-translate-y-1/2 -left-10 transition-transform  hover:scale-105 active:scale-110"
        />
      </button>
      <button on:click={() => handleArrowClick("next")}>
        <IconArrowLeft
          width="32"
          class="stroke-purple-secondary !w-auto !h-auto !rotate-180 stroke-9 cursor-pointer top-1/2 !-translate-y-1/2 -right-10 transition-transform  hover:scale-105 active:scale-110"
        />
      </button>
    </div>
  </div>
...
<style lang="postcss">
  .slider-container {
    @apply w-auto m-auto relative h-[320px] bg-white;
  }
  ...
  .slider-container .slider .slide {
    @apply inline-block md:w-[80%] w-[60%] h-[320px] absolute left-1/2 top-1/2 bg-white;
