<script lang="ts">
    ...
    // $: if ($filtersStore) {
    //   values = getFilterValueById('weekRange') || [min, max];
    // }
  </script>
  <!-- setFilterValueById('weekRange', e.detail.values);  -->
  <RangeSlider class="w-full min-w-32"  on:stop={(e) => { }} float range {values} max={7} />
